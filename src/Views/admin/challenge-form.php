...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="bg-white p-6 rounded-lg shadow-md">
        <form method="POST" enctype="multipart/form-data" action="<?= $action === 'create' ? '/admin/challenges/create' : '/admin/challenges/update/' . $challenge['id'] ?>">
            <!-- CSRF Token -->
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <input type="text" id="title" name="title" value="<?= $action === 'edit' ? htmlspecialchars($challenge['title']) : (isset($old['title']) ? htmlspecialchars($old['title']) : '') ?>" required class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                </div>
                
                <div>
                    <label for="category_id" class="block text-sm font-medium text-gray-700 mb-1">Category *</label>
                    <select id="category_id" name="category_id" required class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
            ...
            <div class="mb-6">
                ...
                <textarea id="description" name="description" rows="6" required class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"><?= $action === 'edit' ? htmlspecialchars($challenge['description']) : (isset($old['description']) ? htmlspecialchars($old['description']) : '') ?></textarea>
            </div>
            
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                <div>
                    <label for="difficulty" class="block text-sm font-medium text-gray-700 mb-1">Difficulty</label>
                    <select id="difficulty" name="difficulty" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                ...
                <div>
                    ...
                    <input type="datetime-local" id="start_date" name="start_date" value="<?= $action === 'edit' ? str_replace(' ', 'T', substr($challenge['start_date'], 0, 16)) : (isset($old['start_date']) ? str_replace(' ', 'T', substr($old['start_date'], 0, 16)) : date('Y-m-d\TH:i')) ?>" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                </div>
                
                <div>
                    <label for="end_date" class="block text-sm font-medium text-gray-700 mb-1">End Date</label>
                    <input type="datetime-local" id="end_date" name="end_date" value="<?= $action === 'edit' ? str_replace(' ', 'T', substr($challenge['end_date'], 0, 16)) : (isset($old['end_date']) ? str_replace(' ', 'T', substr($old['end_date'], 0, 16)) : date('Y-m-d\TH:i', strtotime('+7 days'))) ?>" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                </div>
            </div>
            
            <div class="mb-6">
                <label for="rules" class="block text-sm font-medium text-gray-700 mb-1">Rules</label>
                <textarea id="rules" name="rules" rows="4" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"><?= $action === 'edit' ? htmlspecialchars($challenge['rules']) : (isset($old['rules']) ? htmlspecialchars($old['rules']) : '') ?></textarea>
            </div>
            
            <div class="mb-6">
                <label for="submission_guidelines" class="block text-sm font-medium text-gray-700 mb-1">Submission Guidelines</label>
                <textarea id="submission_guidelines" name="submission_guidelines" rows="4" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"><?= $action === 'edit' ? htmlspecialchars($challenge['submission_guidelines']) : (isset($old['submission_guidelines']) ? htmlspecialchars($old['submission_guidelines']) : '') ?></textarea>
            </div>
